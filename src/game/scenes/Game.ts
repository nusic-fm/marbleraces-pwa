...
export default class Game extends Phaser.Scene {
    ...
    if (randomObstaclePosition && randomObstacle) {
      const target = this.matter.add
        .sprite(
          ...
          {
            shape:
              obstacleShapes[randomObstacle as keyof typeof obstacleShapes],
            friction: 0,
            frictionAir: 0,
            frictionStatic: 0,
          }
  ...
  create() {
    ...
    var obstacleShapes = this.cache.json.get("obstacles_shapes");
  ...
  update(time: number, delta: number): void {
    ...
    this.horizontalCrossRightRotation.map((rs) => rs.setAngle(rs.angle + 2.5));
    this.horizontalCrossLeftRotation.map((rs) => rs.setAngle(rs.angle - 2.5));

    // Bars up/down motion
    this.motionTimeForUpDownWard += delta;
    this.upDownMotionElems.map(
      ({ matter, startX, startY, moveSpeed, maxBottom, maxTop, direction }) => {
        ...
        if (direction === "right") {
          const newX = startX + offset * Math.sin(Phaser.Math.DegToRad(7.1));
          const newY = startY - offset * Math.cos(Phaser.Math.DegToRad(7.1));
