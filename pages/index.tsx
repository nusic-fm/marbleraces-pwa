...
const Index = () => {
  ...
  const [bgNo, setBgNo] = useState(11);
  ...
  return (
    <Stack height={"100vh"}>
      {user && (
        <Box display={"flex"} justifyContent="end" width={"100%"} p={2}>
                    ...
                    {["stars_01.png", "snow.png"].map((name) => (
                      <Avatar
                        ...
                        sx={{
                          ...
                          cursor: "pointer",
                          outline:
                            name === selectedSkin ? "1px solid white" : "unset",
                        }}
      ...
      <Dialog open={activeStep === 1 && !user}>
        ...
        <DialogContent>
          <EmailLink
            url={`http://localhost:3000?coverId=${selectedCoverDoc?.id}`}
          />
        </DialogContent>
      </Dialog>
    </Stack>
